...
const SLAM_RADIUS_LANDSCAPE := 82.0
...
const SLAM_RADIUS_PORTRAIT := 66.0
...
func _add_sky_fill_portrait() -> void:
	...
	# topo do fundo do Cemiterio preenche o espaco extra revelado acima da
	# sala quando content_scale_size passa a ser 180x320.
	var top_color := Color("15192a")
	var deep_color := Color("06070d")
...
func _spawn_enemies() -> void:
	...
	boss_actor = _spawn_actor("OGRO", "enemy", "ogre", Vector2(boss_x, ACTOR_GROUND_Y), Color("a8c9a0"))
...
func _build_hud() -> void:
	...
	if is_portrait:
		var refs := BossHudPortrait12.build_hud(canvas, "OGRO")
