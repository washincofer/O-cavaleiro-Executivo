...
func _controlled_tick() -> void:
	...
	if Input.is_action_just_pressed("attack"):
		if is_player:
			_attack_nearest(true)
		else:
			controller.activate_companion_ability(self)

func _ally_follow_tick() -> void:
	var leader: ReceptionActor = controller.get_active_actor()
	if not is_instance_valid(leader) or leader == self:
		velocity = Vector2.ZERO
		return
...
func _enemy_ai_tick(delta: float) -> void:
...
func _move_toward_point(destination: Vector2) -> void:
	...
	for other in controller.actors:
		if is_instance_valid(other) and other != self and other.alive:
			...
			if d < 20.0 and d > 0.1:
				direction += other.global_position.direction_to(global_position) * (20.0 - d) / 20.0
...
func take_damage(amount: int, source: ReceptionActor) -> void:
	if not alive or source == self or source.team == team:
		return
...
func _draw() -> void:
	...
	if is_controlled and team == "ally":
		draw_arc(Vector2(0, -8), 14.0, 0, TAU, 28, Color("ffe26f"), 3.0)
	...
	if role == "ranged":
		draw_line(Vector2(-7, -2), Vector2(7, 2), Color(0.75, 0.5, 0.18), 3.0)

	if role == "coordinator":
		draw_arc(Vector2(0, -8), 11.0, 0, TAU, 20, Color(0.72, 0.35, 0.95), 2.0)
