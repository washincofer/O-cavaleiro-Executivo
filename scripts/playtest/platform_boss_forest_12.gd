...
## Sprint 15: 3a fase de boss fight (clareira da Floresta, fundo montado a
## partir do pack "seamless patterns"), mesmo formato "sala unica" das
## Ruinas — o trio selecionado enfrenta um unico inimigo gigante, o SATYR
## (pack "SATYR_sprite_sheet_"), cuja folha 320x352 (10 colunas x 11 linhas
## de 32x32) foi recortada por linha: Idle (linha 0, 6 frames), Walk (linha
## 1, 8), Attack (linha 3, 7 — golpe de espada), Hurt (linha 7, 4 — flash) e
## Death (linha 6, 10).
##
## Mecanica identica a Ruinas (investida com aviso/interrupcao) — apenas
## reskinada com textos do Satyr — e recompensa: vencer libera o PALADINO
...
func _process(delta: float) -> void:
	...
	if not completed and active_enemies <= 0 and total_enemies > 0:
		completed = true
		report_event("VITORIA — O SATYR FOI DERROTADO")
...
# --- Mundo: sala unica com fundo da clareira da Floresta ---------------------
...
func _add_ledge(rect: Rect2) -> void:
	...
	# Plataforma de galho grosso apoiada em um tronco, coerente com a
	# clareira da floresta (em vez do pilar de entulho das Ruinas).
